...
(function() {
  var Main, Tile, Tudor, tudor, zz, ª, ªA, ªB, ªC, ªE, ªF, ªN, ªO, ªR, ªS, ªU, ªV, ªX, ªex, ªhas, ªredefine, ªtype, ªuid,
  ...
  tudor.add([
    "01 Actile Constructor Usage", tudor.is, "The class and instance are expected types", "The class is a function", ªF, function() {
      return Main;
    }, "An instance is an object", ªO, function() {
      return new Main({
        tiles: 1
      });
    }, "Constructor must be passed an object", tudor["throw"], "If defined, `config` must be an object", "`config` is type 'number' not 'object'", function() {
      return new Main(123);
    }, "`config.tiles` must be a number, or an array of objects", "`actile.tiles` is null", "`config.tiles` is type 'null' not 'array|number'", function() {
      var actile;
      return actile = new Main({
        tiles: null
      });
    }, "`actile.tiles` is an array of booleans", "`config.tiles` contains type 'boolean'", function() {
      var actile;
      return actile = new Main({
        tiles: [true, false]
      });
    }, "If `config.tiles` is a number, default Tiles are generated", function() {
      return [
        new Main({
          tiles: 3
        })
      ];
    }, tudor.is, "`actile.tiles` is an array", ªA, function(a) {
      var actile;
      return actile = a.tiles;
    }, tudor.equal, "`actile.tiles` contains the expected number of elements", 3, function(a) {
      return a.tiles.length;
    }, "The `actile.tiles` elements are objects", ªO, function(a) {
      return typeof a.tiles[0];
    }, "The `actile.tiles` elements are Tile instances", true, function(a) {
      return a.tiles[0] instanceof Tile;
    }, "The `actile.tiles` elements have square aspect ratios", 1, function(a) {
      return a.tiles[0].ar;
    }, "If `config.tiles` is an array of objects, config Tiles are generated", function() {
      return [
        new Main({
          tiles: [
            {
    ...
    }, "`actile.tiles` contains the expected number of elements", 2, function(a) {
      return a.tiles.length;
    }, "The `actile.tiles` elements have the expected aspect ratios", '0.5 3.5', function(a) {
      return a.tiles[0].ar + ' ' + a.tiles[1].ar;
    }, tudor["throw"], "The array of objects must be valid Tile configs", "`ar` is 'regexp' not 'number'", function() {
      return new Main({
        tiles: [
          {
            ar: /abc/
    ...
    }, "If `config.tiles` is an array of Tiles, no generation is needed", tudor.equal, function() {
      return [
        new Main({
          tiles: [
            new Tile({
    ...
    }, "`actile.tiles` contains the expected number of elements", 3, function(a) {
      return a.tiles.length;
    }, "The `actile.tiles` elements have the expected aspect ratios", '4.6 0.1 55', function(a) {
      return a.tiles[0].ar + ' ' + a.tiles[1].ar + ' ' + a.tiles[2].ar;
    }, "`config.tiles` can be an array of Tiles, objects, and `undefined`", function() {
      return [
        new Main({
          tiles: [
            new Tile({
    ...
    }, "`actile.tiles` contains the expected number of elements", 4, function(a) {
      return a.tiles.length;
    }, "The `actile.tiles` elements have the expected aspect ratios", '4.6 1 1 22', function(a) {
      return a.tiles[0].ar + ' ' + a.tiles[1].ar + ' ' + a.tiles[2].ar + ' ' + a.tiles[3].ar;
    }, "`config.tiles` can be an empty array", function() {
      return [
        new Main({
          tiles: []
        })
      ];
    }, "`actile.tiles` contains no elements", 0, function(a) {
      return a.tiles.length;
    }
  ]);
